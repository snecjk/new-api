...
| 权限 | root（`sudo`） |
| 架构 | x86_64 / ARM64 |
| 网络 | 能访问 `download.docker.com` 与 `registry-1.docker.io`（东京 ap-northeast-1 直连无碍） |
...
- 自检：`docker version` + `docker compose version` + `docker run --rm hello-world`
...
> `hello-world` 拉自 Docker Hub。海外直连无碍；若设了国内 Hub 加速器且该加速器无 `hello-world`，可跳过此步。
> 最后一条 `docker pull` 确认能拉到后续部署 new-api 要用的镜像。
...
| `docker run hello-world` 拉取失败 | Docker Hub 抖动；`docker pull calciumion/new-api:latest` 单独验证，或设 `DOCKER_REGISTRY_MIRROR` 加速器重跑 |
...
Docker 装好后，按 [`deploy-newapi.md`](deploy-newapi.md) 部署 new-api（复用国内 `140.143.183.34`
的 MySQL+Redis，海外以 `slave` 角色运行）：
